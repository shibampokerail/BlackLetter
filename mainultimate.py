...
def upload_page():
    """
    Renders the new, modern upload page as the homepage.
    """
...
def viewer_page():
    """
    Renders the PDF viewer and analysis page.
    Note: The template file must be renamed to viewer.html
    """
...
def upload_document():
    """
    Handles file uploads and triggers a re-index.
...
def uploaded_file(filename):
    """
    Serves a specific file from the upload folder for the viewer.
    (This route remains the same)
    """
...
def analyze_document():
    """
    ✨ UPDATED: Takes a filename, reads the text, and asks the AI to return a
    summary, outdated sections, and improvements in a single JSON object.
    """
    ...
    try:
        ...
        # Limit the text sent for analysis to avoid exceeding token limits for very large docs
        # We can use the first ~15-20 pages worth of text as a reasonable proxy.
        text_limit = 30000
    ...
    # ✨ UPDATED PROMPT: Asks for all three items in one structured JSON.
    PROMPT_FOR_ANALYSIS = f"""
        ...
        **CRITICAL**: You MUST return your analysis as a single, valid JSON object.
        ...
        {{
          "summary": "This document outlines the hospital's policy regarding the use of information technology resources. It covers acceptable use for internet, email, and network access, and details the security protocols employees must follow to protect patient data and ensure HIPAA compliance. The policy applies to all staff, contractors, and volunteers.",
          "outdated": [
            {{
              "quote": "All employees must use Internet Explorer for web browsing.",
              "explanation": "Internet Explorer is a deprecated browser with security vulnerabilities and should be replaced with a modern browser standard like Chrome or Edge."
            }}
          ],
          "improvements": [
            {{
    ...
    try:
        # ✨ It's highly recommended to use a model that supports JSON output mode for reliability.
        # This improves the chances of getting valid JSON every time.
        ...
        # The response should now be clean JSON directly from the model
        analysis_result = json.loads(response.text)

        return jsonify(analysis_result)

    except (json.JSONDecodeError, ValueError) as e:
        # This block catches errors if the AI fails to return valid JSON.
